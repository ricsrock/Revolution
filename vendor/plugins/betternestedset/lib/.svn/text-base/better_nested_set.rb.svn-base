...
#            }
      ...
      module InstanceMethods
        ...
        def ancestors(options=nil)
          self_and_ancestors(options) - [self]
        end
        
        # Returns an array of all parents plus self, starting with the root.
        def self_and_ancestors(options=nil)
          options && options[:scope] ? leaf_scope_condition = options[:scope] : leaf_scope_condition = "1 = 1"     
          base_set_class.find(:all, :conditions => "#{scope_condition} AND (#{self[left_col_name]} BETWEEN #{left_col_name} AND #{right_col_name} AND NOT (#{left_col_name} + 1 = #{right_col_name}))
                                                                       OR (#{scope_condition} AND #{self[left_col_name]} BETWEEN #{left_col_name} AND #{right_col_name} AND (#{left_col_name} + 1 = #{right_col_name}) AND #{leaf_scope_condition})", :order => left_col_name )
        end
        
        # Returns all the children of this node's parent, except self.
        def siblings(options=nil)
          self_and_siblings(options) - [self]
        end
        
        # Returns all the children of this node's parent, including self.
        def self_and_siblings(options=nil)
          options && options[:scope] ? leaf_scope_condition = options[:scope] : leaf_scope_condition = "1 = 1"     
          if self[parent_col_name].nil? || self[parent_col_name].zero?
            [self]
          else
            base_set_class.find(:all, :conditions => "#{scope_condition} AND #{parent_col_name} = #{self[parent_col_name]} AND NOT (#{left_col_name} + 1 = #{right_col_name})
                                                                         OR (#{scope_condition} AND #{parent_col_name} = #{self[parent_col_name]} AND (#{left_col_name} + 1 = #{right_col_name}) AND #{leaf_scope_condition})", :order => left_col_name)
          end
        end
        ...
        # Mod: Pass :scope => "archived_on..." to scope leaves only
        def full_set(options=nil,special=nil)
          ...
          options && options[:scope] ? leaf_scope_condition = options[:scope] : leaf_scope_condition = "1 = 1"     
          base_set_class.find(:all, :conditions => "#{scope_condition} #{exclude_str} AND (#{left_col_name} BETWEEN #{self[left_col_name]} AND #{self[right_col_name]} AND NOT (#{left_col_name} + 1 = #{right_col_name}))
                                                                                      OR (#{scope_condition} AND #{left_col_name} BETWEEN #{self[left_col_name]} AND #{self[right_col_name]} AND (#{left_col_name} + 1 = #{right_col_name}) AND #{leaf_scope_condition} )",
                                                                                      :order => left_col_name)
        end
        
        # Returns all children and nested children.
        # Pass :exclude => item, or id, or [items or id] to exclude one or more items *and* all of their descendants.
        def all_children(options=nil,special=nil)
          full_set(options,special) - [self]
        end
        
        # Returns this record's immediate children.
        def children(options=nil)
          options && options[:scope] ? leaf_scope_condition = options[:scope] : leaf_scope_condition = "1 = 1"     
          base_set_class.find(:all, :conditions => "#{scope_condition} AND #{parent_col_name} = #{self.id} AND NOT (#{left_col_name} + 1 = #{right_col_name})
                                                                       OR (#{scope_condition} AND #{parent_col_name} = #{self.id} AND (#{left_col_name} + 1 = #{right_col_name}) AND #{leaf_scope_condition} )", :order => left_col_name)
        end
