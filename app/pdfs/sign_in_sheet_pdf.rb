class SignInSheetPdf < Prawn::Document
  
  def initialize(smart_group, view)
    super(top_margin: 70)
  ...
  def smart_group_name
    text "Smart Group: #{@smart_group.name}", style: :bold
  ...
  def people
    ...
    columns = %w[FirstName 1stService 2ndService]
    data = []
    data << columns
    @smart_group.result.order('last_name ASC, first_name ASC').each do |person|
      attributes = []
      attributes << person.full_name
      attributes << "_____________________________________"
      attributes << "_____________________________________"
